...
public class MetaballBuilder
{
    ...
    public string CreateMesh(MetaballCellClusterInterface rootCell, Transform root, float powerThreshold, float gridCellSize, Vector3 uDir, Vector3 vDir, Vector3 uvOffset, out Mesh out_mesh, MetaballCellObject cellObjPrefab = null, bool bReverse = false, Bounds ? fixedBounds = null, bool bAutoGridSize = false, float autoGridQuarity = 0.2f, bool withColor = true)
    {
        ...
        BuildMetaballMesh(mesh, bounds.center, bounds.extents, gridCellSize, points, powerThreshold, bReverse, uDir, vDir, uvOffset, false, withColor);
    ...
    public string CreateMeshWithSkeleton(SkinnedMetaballCell rootCell, Transform root, float powerThreshold, float gridCellSize, Vector3 uDir, Vector3 vDir, Vector3 uvOffset, out Mesh out_mesh, out Transform[] out_bones, MetaballCellObject cellObjPrefab = null, bool bReverse = false, Bounds? fixedBounds = null, bool bAutoGridSize = false, float autoGridQuarity = 0.2f, bool withColor = true)
    {
      ...
      //  float gridCellSize = rootCell.radius * Mathf.Lerp(0.3f, 0.1f, Mathf.Clamp01(detailLevel));
        ...
        BuildMetaballMesh(mesh, bounds.center, bounds.extents, gridCellSize, points, powerThreshold, bReverse, uDir, vDir, uvOffset, true, withColor);
...
//        List<BoneWeight> boneWeightList = new List<BoneWeight>();
    ...
    void BuildMetaballMesh( Mesh mesh, Vector3 center, Vector3 extent, float cellSize, MetaballPointInfo [] points, float powerThreshold, bool bReverse, Vector3 uDir, Vector3 vDir, Vector3 uvOffset, bool withSkin, bool withColor)
    {         
	    if(!__bCubePatternsInitialized)
	    {
		    __InitCubePatterns();
        ...
        Vector4[] weightedColorMap = new Vector4[resolutionX * resolutionY * resolutionZ];
        ...
        foreach( MetaballPointInfo pointInfo in points )
        {
            ...
            for( int cellZ = minCellZ; cellZ <= maxCellZ; ++cellZ )
            {
                for( int cellY = minCellY; cellY <= maxCellY; ++cellY )
                {
                    ...
                    {
                        int index = cellX + cellY * gridStrideY + cellZ * gridStrideZ;

                        Vector3 cellPos = positionMap[index];
                        
                        float power = CalcPower(cellPos - pointInfo.center, pointInfo.radius, pointInfo.density);

						powerMap[index] += power;
                        ...
                        if (power > 0.0f)
                        {
                            ...
                            if( withSkin )
                            {
                                BoneWeight bw = boneWeightMap[index];
                            ...
                            if( withColor )
                            {
                                Vector4 weightedColor = weightedColorMap[index];
                                Vector4 pointColor = new Vector4( pointInfo.baseColor.x*power, pointInfo.baseColor.y*power, pointInfo.baseColor.z*power, power);
                                weightedColorMap[index] = weightedColor + pointColor;
                            }
                        }
                    }
                }
				    ...
				    for( int dir=0; dir<3 && vertexCount < _maxVertexCount-1; ++dir )
				    {
					    ...
					    if( z+dz < gridResolutionZ && y+dy < gridResolutionY && x+dx < gridResolutionX )
					    {
						    ...
						    if( ( p0 - threshold ) * ( p1 - threshold ) < 0.0f )
						    {
                                ...
                                if (withColor)
                                {
                                    Vector4 weightedColor = weightedColorMap[idx1] * alpha + weightedColorMap[idx0] * (1.0f - alpha);
                                    if (weightedColor.w <= 0.0f)
                                    {
                                        colorList.Add(Color.white);
                                    }
                                    else
                                    {
                                        colorList.Add(new Color(weightedColor.x / weightedColor.w, weightedColor.y / weightedColor.w, weightedColor.z / weightedColor.w));
                                    }
                                }
